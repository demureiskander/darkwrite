...
describe("notesUiSlice rename dialog", () => {
  it("opens for one object and clears its target when closed", () => {
    const opened = notesUiSlice.reducer(
      initialNotesUiState,
      notesUiSlice.actions.showRenameNoteDialog({ noteId: "note-1" }),
    );

    expect(opened.renameNoteDialog).toEqual({
      open: true,
      noteId: "note-1",
    });

    const closed = notesUiSlice.reducer(
      opened,
      notesUiSlice.actions.closeRenameNoteDialog(),
    );
    expect(closed.renameNoteDialog).toEqual({ open: false, noteId: null });
  });
});
