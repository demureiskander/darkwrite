...
describe("createNote", () => {
  ...
  it("opens rename only after persistence succeeds", async () => {
    selectWorkspace();

    await store.dispatch(createNote({ parentId: null, renameAfter: true }));

    const created = createMock.mock.calls[0][0];
    expect(store.getState().noteUi.renameNoteDialog).toEqual({
      open: true,
      noteId: created.id,
    });
  });

  it("does not open rename when persistence fails", async () => {
    selectWorkspace();
    createMock.mockReturnValue(dwErrAsync("boom"));
...
describe("duplicateNote", () => {
  ...
  it("duplicates selected documents sequentially and ignores folders", async () => {
    ...
    seed(first, folder, second);
    seedContent(first.id, makeContent());
    seedContent(second.id, makeContent());

    const result = await store.dispatch(
      duplicateNotes([first.id, folder.id, second.id, first.id]),
    );

    expect(result.isOk()).toBe(true);
    expect(result._unsafeUnwrap()).toHaveLength(2);
    expect(createMock).toHaveBeenCalledTimes(2);
    const copies = layer().filter((note) => note.title.endsWith("(copy)"));
    expect(copies.map((note) => note.title)).toEqual([
      "First (copy)",
      "Second (copy)",
    ]);
    expect(new Set(copies.map((note) => note.orderHint)).size).toBe(2);
  });

  it("undoes and redoes a duplicate command", async () => {
    const source = makeNote({ id: "source", title: "Source" });
    seed(source);
    seedContent(source.id, makeContent());

    await store.dispatch(duplicateNotes([source.id]));
    expect(layer()).toHaveLength(2);

    expect(await undoStructuralAction(WORKSPACE_ID)).toBe(true);
    expect(layer().map((note) => note.id)).toEqual([source.id]);

    expect(await redoStructuralAction(WORKSPACE_ID)).toBe(true);
    expect(layer()).toHaveLength(2);
    expect(
      layer().filter((note) => note.title === "Source (copy)"),
    ).toHaveLength(1);
  });
});
...
describe("reorderFavorite", () => {
  ...
  it("pins folders in the same ordered quick-access list", async () => {
    const folder = makeNote({ kind: NoteKind.Folder });
    seed(folder);

    await store.dispatch(reorderFavorite(folder.id));

    expect(selectNoteById(store.getState(), folder.id)?.isFavorite).toBe(true);
    expect(favIds()).toEqual([folder.id]);
  });
...
describe("toggleFavorites", () => {
  ...
  it("pins every selected item in selection order", async () => {
    ...
    seed(
      existing,
      makeNote({ id: "folder", kind: NoteKind.Folder }),
      makeNote({ id: "document" }),
    );

    await store.dispatch(toggleFavorites(["folder", "document"]));

    expect(
      selectFavorites(store.getState(), WORKSPACE_ID).map((note) => note.id),
    ).toEqual(["existing", "folder", "document"]);
  });

  it("keeps pinned items pinned when a mixed selection is pinned", async () => {
    seed(
      makeNote({ id: "pinned", isFavorite: true, favoriteOrderHint: "a1" }),
      makeNote({ id: "plain" }),
    );

    await store.dispatch(toggleFavorites(["pinned", "plain"]));

    expect(selectNoteById(store.getState(), "pinned")?.isFavorite).toBe(true);
    expect(selectNoteById(store.getState(), "plain")?.isFavorite).toBe(true);
  });

  it("unpins the whole selection when every item is pinned", async () => {
    seed(
      makeNote({ id: "one", isFavorite: true, favoriteOrderHint: "a1" }),
      makeNote({ id: "two", isFavorite: true, favoriteOrderHint: "a2" }),
    );

    await store.dispatch(toggleFavorites(["one", "two"]));

    expect(selectNoteById(store.getState(), "one")?.isFavorite).toBe(false);
    expect(selectNoteById(store.getState(), "two")?.isFavorite).toBe(false);
  });

  it("undoes and redoes a pin change", async () => {
    seed(makeNote({ id: "one" }), makeNote({ id: "two" }));
    await store.dispatch(toggleFavorites(["one", "two"]));
    ...
    expect(await redoStructuralAction(WORKSPACE_ID)).toBe(true);
    expect(selectNoteById(store.getState(), "one")?.isFavorite).toBe(true);
    expect(selectNoteById(store.getState(), "two")?.isFavorite).toBe(true);
  });
});
...
describe("trash + favorite status thunks", () => {
  ...
  describe("moveToTrash", () => {
    ...
    it("trashes every descendant when moving a folder to trash", async () => {
      const folder = makeNote({ id: "folder", kind: NoteKind.Folder });
      const child = makeNote({ id: "child", parentId: folder.id });
      const grandchild = makeNote({ id: "grandchild", parentId: child.id });
      const sibling = makeNote({ id: "sibling" });
      seed(folder, child, grandchild, sibling);

      await store.dispatch(moveToTrash(folder.id));

      expect(selectNoteById(store.getState(), folder.id)?.isTrashed).toBe(true);
      expect(selectNoteById(store.getState(), child.id)?.isTrashed).toBe(true);
      expect(selectNoteById(store.getState(), grandchild.id)?.isTrashed).toBe(
        true,
      );
      expect(selectNoteById(store.getState(), sibling.id)?.isTrashed).toBe(
        false,
      );
    });

    it("undoes and redoes moving a folder tree to trash", async () => {
      const folder = makeNote({ id: "folder", kind: NoteKind.Folder });
      const child = makeNote({ id: "child", parentId: folder.id });
      seed(folder, child);
      await store.dispatch(moveToTrash(folder.id));
      ...
      expect(selectNoteById(store.getState(), child.id)?.isTrashed).toBe(false);
    ...
    it("trashes multiple selected objects without duplicating descendants", async () => {
      const folder = makeNote({ id: "folder", kind: NoteKind.Folder });
      const child = makeNote({ id: "child", parentId: folder.id });
      const document = makeNote({ id: "document" });
      const untouched = makeNote({ id: "untouched" });
      seed(folder, child, document, untouched);

      await store.dispatch(
        moveManyToTrash([folder.id, child.id, document.id, folder.id]),
      );

      expect(selectNoteById(store.getState(), folder.id)?.isTrashed).toBe(true);
      expect(selectNoteById(store.getState(), child.id)?.isTrashed).toBe(true);
      expect(selectNoteById(store.getState(), document.id)?.isTrashed).toBe(
        true,
      );
      expect(selectNoteById(store.getState(), untouched.id)?.isTrashed).toBe(
        false,
      );
      const patches = patchMock.mock.calls[0]?.[0] ?? [];
      expect(patches.map((item: { id: string }) => item.id).sort()).toEqual([
        "child",
        "document",
        "folder",
      ]);
