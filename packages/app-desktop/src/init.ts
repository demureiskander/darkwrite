...
import { type IZoomService, ZoomService } from "./lib/zoom";
...
async function showMainWindow(
  ...
  zoomService.applyStored(win.webContents);
  ...
  initAppMenu(zoomService, settings.client.shortcuts);
}
...
export async function init() {
  ...
  const zoomService = ZoomService(settingsService);
  ...
  const apiBridge: HandlerImplements<DarkwriteIPCBridge> = {
    ...
    settings: SettingsAPI(settingsService, (settings) => {
      setLanguage(settings.client.language);
      initAppMenu(zoomService, settings.client.shortcuts);
    }),
