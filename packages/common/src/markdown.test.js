...
describe("properties to markdown", () => {
    const isFrontmatter = (str) => str.startsWith("---\n") && str.endsWith("---\n");
    const parseBack = (frontmatter) => parseYaml(frontmatter.substring(3, frontmatter.length - 4));
    it.each([
        {
            property: {
        ...
        const frontmatter = MarkdownConverter.propertiesToFrontmatter({ Property: property }, ["Property"]), parsedYaml = parseBack(frontmatter);
    ...
    it("preserves order of properties", () => {
        ...
        }, order = ["b", "c", "a"], frontmatter = MarkdownConverter.propertiesToFrontmatter(properties, order);
        expect(isFrontmatter(frontmatter)).toBe(true);
