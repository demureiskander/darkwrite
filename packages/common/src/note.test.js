import { isDescendant, Note, NoteProperty, PropertyType, PropertyUpdater, resolveUpperTree, } from "./note";
...
describe("isDescendant", () => {
    it("should break gracefully on circular references", () => {
        ...
        expect(isDescendant("a", "d", notes), "what happens if a test candidate does not exist at all?").toBe(false);
...
describe("note property tests", () => {
    ...
    describe("setNoteProperty", () => {
        it.each(testProperties)("should create a note property on a note that has no properties", (property) => {
        ...
        it.each(testProperties)("should create a property on a note with existing properties", (property) => {
        ...
        it.each(testProperties)("should update an existing property", (property) => {
        ...
        // test reconciliation
        it.each(testProperties)("should add to order if property is out of order", (property) => {
            const name = "source";
            const note = Note._test({
                properties: { [name]: property },
                propertyOrder: [],
    ...
    describe("renameNoteProperty", () => {
        it.each(testProperties)("should rename a property", (property) => {
            ...
            const result = PropertyUpdater.renameNoteProperty(note, src, dest)._unsafeUnwrap();
    ...
    describe("deleteNoteProperty", () => {
        it("deletes a property", () => {
            ...
            const result = PropertyUpdater.deleteNoteProperty(note, name)._unsafeUnwrap();
            expect(result).toStrictEqual({
                id: note.id,
                propertyOrder: [],
                properties: {},
        ...
        it("doesnt touch other properties", () => {
            ...
            const result = PropertyUpdater.deleteNoteProperty(note, "a")._unsafeUnwrap();
            expect(result).toStrictEqual({
                id: note.id,
                propertyOrder: ["b"],
    ...
    describe("reorderNoteProperty", () => {
        it.each([
            ...
            const diff = PropertyUpdater.reorderNoteProperty(note, src, dest, "before")._unsafeUnwrap();
        ...
        it.each([
            ...
            const diff = PropertyUpdater.reorderNoteProperty(note, src, dest, "after")._unsafeUnwrap();
            expect(diff.propertyOrder).toEqual(expected);
        });
        it("should no-op when source = dest", () => {
            ...
            const result_after = PropertyUpdater.reorderNoteProperty(note, "a", "a", "after")._unsafeUnwrap();
            const result_before = PropertyUpdater.reorderNoteProperty(note, "a", "a", "before")._unsafeUnwrap();
        ...
        it("should reconcile when source doesn't exist in order", () => {
            ...
            const result = PropertyUpdater.reorderNoteProperty(note, "d", "a", "after")._unsafeUnwrap();
        ...
        it("should err when source doesn't exist in properties", () => {
            ...
            const result = PropertyUpdater.reorderNoteProperty(note, "d", "a", "after");
            expect(result.isErr()).toBe(true);
        });
        it("should reconcile when dest doesn't exist in order", () => {
            ...
            const result = PropertyUpdater.reorderNoteProperty(note, "a", "d", "after")._unsafeUnwrap();
        ...
        it("should err when dest doesn't exist in properties", () => {
            ...
            const result = PropertyUpdater.reorderNoteProperty(note, "a", "d", "after");
            expect(result.isErr()).toBe(true);
        });
    });
});
