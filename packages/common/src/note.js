...
export const serializeRange = (range) => JSON.stringify({
    from: range.from?.toISOString(),
    to: range.to?.toISOString(),
});
export const deserializeRange = (serialized) => parseJson(serialized).map((result) => ({
...
export function getDefaultNoteProperty(type) {
...
export const Note = {
    ...
    hasProperty: (note, propertyName) => Object.hasOwn(note.properties, propertyName),
};
 ...
 */
export function resolveUpperTree(id, notes) {
    const list = [];
    if (!notes[id] || !("parentId" in notes[id]))
        return [];
 ...
 */
export function isDescendant(potentialChildId, potentialParentId, notesMapOrGetter) {
    const visited = new Set();
    if (potentialChildId === potentialParentId)
        return "CIRCULAR";
    const getNote = (id) => {
        if (typeof notesMapOrGetter === "function")
            return notesMapOrGetter(id);
        else
            return notesMapOrGetter[id];
    };
 ...
 * @returns "CIRCULAR" if child and parent are in a circular reference, true if is a descendant, false in all other cases
 */
export async function isDescendantAsync(potentialChildId, potentialParentId, getNote) {
    const visited = new Set();
    if (potentialChildId === potentialParentId)
        return "CIRCULAR";
    let currentNoteId = potentialChildId;
 ...
 */
export const cleanNoteTitle = (title) => removeLinebreaks(title);
...
export const stableSortByOrderKeyFn = (key = "orderHint") => (a, b) => {
 ...
 */
function setNoteProperty(note, propertyName, property) {
 ...
 * @returns a diff to perform updates
 */
function renameNoteProperty(note, oldName, newName) {
    if (!Note.hasProperty(note, oldName))
        return dwErr(`Property ${oldName} does not exist in "${note.title}"`);
    if (newName.trim() === "")
        return dwErr(`Property name cannot be empty.`);
    ...
    if (idx === -1)
        ...
        diff.propertyOrder[idx] = newName;
    return ok(diff);
}
/**
 ...
 */
function deleteNoteProperty(note, propertyName) {
    if (!Note.hasProperty(note, propertyName))
        return dwErr(`Property "${propertyName}" does not exist in "${note.title}"`);
    const diff = PropertyDiff.from(note);
    delete diff.properties[propertyName];
    diff.propertyOrder = diff.propertyOrder.filter((name) => name !== propertyName);
    return ok(diff);
}
function reorderNoteProperty(note, source, dest, placement) {
    if (!Note.hasProperty(note, source) || !Note.hasProperty(note, dest))
        return dwErr("Source or destination property doesn't exist.");
    const diff = PropertyDiff.from(note);
    if (!diff.propertyOrder.includes(source))
        diff.propertyOrder.push(source);
    if (!diff.propertyOrder.includes(dest))
        diff.propertyOrder.push(dest);
    if (source === dest)
        return ok(diff);
    // remove the source
    diff.propertyOrder = diff.propertyOrder.filter((prop) => prop !== source);
    const destIdx = diff.propertyOrder.indexOf(dest);
    diff.propertyOrder.splice(placement === "before" ? destIdx : destIdx + 1, 0, source);
    return ok(diff);
}
